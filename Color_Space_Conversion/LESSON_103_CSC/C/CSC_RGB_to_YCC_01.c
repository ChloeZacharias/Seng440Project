...
//
    ...
    YY = vaddq_u32(YY, add_valueY);
    YY = vaddq_u32(YY, scale); //rounding
    ...
    uint32x4_t CbCb = vaddq_u32(add_valueC, BB_scaled);
    CbCb = vsubq_u32(CbCb, RR_scaled);
    CbCb = vsubq_u32(CbCb, GG_scaled);

    CbCb = vaddq_u32(CbCb, scale); // rounding
